    ...
    if filepath.endswith(".csv"):
        ...
        with open(filepath, mode='r') as f:
            ...
            for value in reader:
                try:
                    framework = value[0]
                    response = requests.get(framework, timeout=5)
                    print(f"[+]{value[0]}  ", response.status_code)
                except requests.ConnectionError:
                    print(f"[+] NOTICE: The Onion Link {value[0]} Is Down \n")
                    continue
    else:
...
def onionheader():
    ...
    if not ".onion" in domain:
        ...
        now = datetime.datetime.now()
        response = session.get(domain, timeout=5)
...
def status_code():
    ...
    now = datetime.datetime.now()
    #if 'https://' in domain:
    page = session.get(domain, timeout=5)
    #else:
    #page = session.get('http://' + domain)
    try:
        if page.status_code == 200:
            good = typer.style("[=]The request has succeeded and the domain is reachable", fg=typer.colors.GREEN)
            typer.echo(good)
            ...
            print("[+]Timestamp: ", now.strftime("%Y-%m-%d %H:%M:%S"))
    #except requests.ConnectionError:
    #                print(f"[+] NOTICE: The Onion Link {domain} Is Down \n")
                    
    except (ConnectTimeout, HTTPError, ReadTimeout, Timeout, ConnectionError):
        print("OOPS!! The Request Timed Out")
    ...
    if filepath.endswith(".csv"):
        with open(filepath, mode='r') as f:
            ...
            for value in reader:
                try:
                    framework = value[0]
                    response = session.get(framework, timeout=5)
